function [sig_regions, acceptance_bounds, data_stat] = CHECK_global_perm_test(varargin)
...
%   [sig_regions, acceptance_bounds] = globalpermtest(group1, group2, alpha_level, statfcn, iterations, ploton)
%
%   Input:
%   group1, group2: in form <dimensions> x <trials> -- required
%   alpha_level:  global acceptance alpha (Default: 0.05)
%   statfcn: handle statistic to compute across trials (Default: @mean(x)(x,2) MUST COMPUTE ACROSS DIM 2)
%   ploton: boolean for plot output (Default: true)
%
%   Output:
...
elseif nargin>7
    ...
    defaults={'','',[],.05,@(x)mean(x,2),5000,true};
...
for i=1:iterations
    ...
    inds1=inds(1:length(inds)/2);
    inds2=inds((length(inds)/2 + 1):end);
...
%     %     axes(ax(2))
%     plot(xvals,p2,'b');
%     title('Condition 1 vs. 2','fontsize',15);
%     axis tight;
%     xlim(xvals([1 end]));
%     
    axes(ax(2))
