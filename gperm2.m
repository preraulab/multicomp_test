...
%Set default row and column bins
if nargin<3
    rbins = 1:C;
end
...
[linear_sigbins, ~, linear_bounds, linear_true_stat] = gperm(g1_redim, g2_redim, [], alpha_level, statfcn, iterations, group1_name, group2_name, false);

%Reshape output
sigbins = reshape(linear_sigbins, R,C);
acceptance_bounds(:,:,1) = reshape(linear_bounds(:,1), R,C);
...
if ploton
    figure
    hold all
    imagesc(rbins, cbins, nanmean(reshape(g1_redim, R, C, N1),3)-nanmean(reshape(g2_redim, R, C, N2),3));
    cx = climscale;
